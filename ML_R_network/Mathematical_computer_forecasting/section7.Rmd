...
- b:Compare the RMSE of the one-step forecasts from the two methods. Which do you prefer?
- c:Check that the residuals from the best method look like white noise.
- d:Now find the test set RMSE, while training the model to the end of 2010. Can you beat the seasonal naïve approach from Exercise 8 in Section 3.7?
...
iii. a seasonal naive method; iv. an STL decomposition applied to the Box-Cox transformed data followed by an ETS model applied to the seasonally adjusted (transformed) data.
...
17. Write down $95 \%$ prediction intervals for an ETS(A,N,N) model as a function of $\ell_{T}, \alpha, h$ and $\sigma$, assuming normally distributed errors.